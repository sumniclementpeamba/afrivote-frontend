...
export default function AuditLogsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-12 relative">
      ...
      <div className="absolute -top-10 -right-10 w-96 h-96 bg-indigo-500/10 dark:bg-indigo-500/15 rounded-full blur-3xl pointer-events-none -z-10" />
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 text-xs font-medium text-slate-600 dark:text-slate-300">
                {logs?.map((log: any) => (
                  <tr key={log.id} className="hover:bg-indigo-50/30 dark:hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider border ${
                        log.action === 'CREATE'
                          ...
                          : log.action === 'UPDATE'
                          ? 'bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-400 border-indigo-200/60 dark:border-indigo-900/40'
                          : log.action === 'DELETE'
